/**
 * HistoryView - Gerencial a renderização do histórico lateral.
 */
class HistoryView {
    constructor() {
        this.container = document.getElementById('historyList');
    }

    render(history, onClick, onDelete) {
        if (!this.container) return;

        if (history.length === 0) {
            this.container.innerHTML = '<p class="empty-history">Nenhuma conversa salva ainda.</p>';
            return;
        }

        this.container.innerHTML = '';
        history.forEach(item => {
            const div = document.createElement('div');
            div.className = 'history-item';
            const date = new Date(item.timestamp).toLocaleDateString('pt-BR', {
                day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit'
            });

            div.innerHTML = `
                <div class="history-item-content">
                    <strong>${item.question}</strong>
                    <span>${date}</span>
                </div>
                <button class="delete-history-btn" title="Excluir" aria-label="Excluir conversa">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">
                        <line x1="18" y1="6" x2="6" y2="18"></line>
                        <line x1="6" y1="6" x2="18" y2="18"></line>
                    </svg>
                </button>
            `;

            div.querySelector('.history-item-content').addEventListener('click', () => onClick(item));
            div.querySelector('.delete-history-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                onDelete(item.id);
            });

            this.container.appendChild(div);
        });
    }
}
